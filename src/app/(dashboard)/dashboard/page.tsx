...
export default function DashboardPage() {
  ...
  const recentReports = [...mockLaporan]
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500">
      ...
      <div className="bg-gray-50 border border-gray-100 rounded-2xl p-4 flex items-center justify-between">
        ...
        <Button variant="outline" className="rounded-full bg-white border-gray-200 shadow-sm text-xs font-semibold">Tugas Aktif &nearr;</Button>
      </div>
        ...
        <div className="xl:col-span-3 space-y-8">
              ...
              {['Day', 'Week', 'Month', 'Year'].map(range => (
                <button
                  ...
                  className={`px-4 py-1.5 rounded-full text-xs font-semibold transition-all ${
                    timeRange === range ? 'bg-white text-black shadow-sm' : 'text-gray-500 hover:text-black'
                  }`}
                >
                  {range}
                </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard 
              title="Total Laporan"
              value={totalLaporan}
              description="Dari 34 (7 hari terakhir)"
            />
            <StatCard 
              title="Menunggu"
              value={menungguCount}
            ...
            <StatCard 
              title="Sedang Proses"
              value={prosesCount}
              description="Ditangani petugas"
            />
            <StatCard 
              title="Selesai"
              value={selesaiCount}
              description="Dari 20 (7 hari terakhir)"
            />
          </div>
        ...
        <div className="space-y-8">
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/petugas" className="text-xs font-semibold hover:underline">Lihat Semua &rarr;</Link>
            </div>
